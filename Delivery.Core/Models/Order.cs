namespace Delivery.Core.Models
{
    public class Order
    {
        ...
        public static (Order Order, string Error) Create(Guid id, double weight, string district, DateTime deliveryTime)
        {
            ...
            if (deliveryTime < DateTime.UtcNow) error = "Datetime can not be less than current datetime";
            if (weight < MIN_WEIGHT) error = $"Weight can not be less than {MIN_WEIGHT}";
            if (weight > MAX_WEIGHT) error = $"Weight can not be more than {MAX_WEIGHT}";
            if (district.Length < MIN_DISTRICT_LEN) error = $"District length can not be less than {MIN_DISTRICT_LEN}";
            if (district.Length > MAX_DISTRICT_LEN) error = $"District length can not be more than {MAX_DISTRICT_LEN}";
